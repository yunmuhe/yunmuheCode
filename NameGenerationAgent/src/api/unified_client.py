...
from .adapters.base_adapter import APIException, BaseAPIAdapter
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # 手动创建API管理器
                from config.api_config import APIManager, PaiouConfig
...
class UnifiedAPIClient:
    ...
    def _initialize_adapters(self):
        ...
        try:
            ...
            logger.info(
                f"已初始化 {len(self.adapters)} 个API适配器: {list(self.adapters.keys())}"
            )
    ...
    def _initialize_router(self):
        ...
        try:
            self.router_strategy = get_router_strategy(
                strategy_name, default_order, weights_raw
            )
        ...
        **kwargs,
        ...
        for index, api_name in enumerate(api_priority):
            ...
            try:
                ...
                result = self.adapters[api_name].generate_names(
                    prompt, **adapter_kwargs
                )
                ...
                logger.info(
                    f"成功使用 {api_name} API生成 {len(result.get('names', []))} 个姓名"
                )
                return result
        ...
        if use_mock_on_failure:
            ...
            # 不使用模拟数据，返回错误
            error_msg = (
                f"所有API调用失败，最后错误: {str(last_error)}"
                if last_error
                else "所有API调用失败"
            )
    ...
    def _build_adapter_kwargs(
        ...
        is_first_attempt_without_preference = (
            preferred_api is None and attempt_index == 0
        )
    ...
    def _get_api_priority(
        self,
        preferred_api: Optional[str] = None,
        context: Optional[Dict[str, Any]] = None,
    ) -> List[str]:
        try:
            return self.router_strategy.get_priority(
                self.adapters, preferred_api, context
            )
        except Exception:
    ...
    def _generate_cache_key(
        self, prompt: str, count: int, kwargs: Dict[str, Any]
    ) -> str:
        """生成缓存键"""
        import hashlib

        # 构建缓存键的组成部分
        key_parts = [prompt, str(count), str(sorted(kwargs.items()))]
    ...
    def test_api_connection(self, api_name: str) -> Dict[str, Any]:
        """测试API连接"""
        if api_name not in self.adapters:
            return {"success": False, "error": f"API {api_name} 不存在"}
        ...
        try:
            ...
            return {"success": False, "error": f"{api_name} API连接失败: {str(e)}"}
